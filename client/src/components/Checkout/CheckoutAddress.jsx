...
const CheckoutAddress = ({ user, address, setAddress, setStep }) => {
    return (
        <div className="bg-white rounded-2xl p-6 animate-fade-in">
            ...
            <Link to="/addresses" onClick={() => scrollTo({ top: 0, behavior: "auto" })} className="mt-6 px-6 py-3 border border-gray-600 text-gray-600 rounded-xl flex-center gap-2">
                Add New Address <PlusIcon className="size-4" />
            </Link>
            <button onClick={() => { setStep("payment"); scrollTo({ top: 0, scroll: "auto" }) }} disabled={!address.address || !address.city} className="mt-6 px-6 py-3 bg-app-green text-white font-semibold rounded-xl hover:bg-app-green-light transition-colors disabled:opacity-50 flex items-center gap-2">
